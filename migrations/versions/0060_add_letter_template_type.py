...
def upgrade():
    ...
    op.execute("ALTER TABLE templates ALTER COLUMN template_type " + "TYPE " + name + " USING template_type::text::" + name)
    op.execute("DROP TYPE " + tmp_name)


def downgrade():
    # Convert 'letter' template into 'email'
    op.execute(tcr.update().where(tcr.c.template_type == "letter").values(template_type="email"))
