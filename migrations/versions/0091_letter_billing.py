...
def upgrade():
    ...
    op.create_index(
        op.f("ix_letter_rate_details_letter_rate_id"),
        "letter_rate_details",
        ["letter_rate_id"],
        unique=False,
    )
    ...
    op.execute(insert_details.format(uuid.uuid4(), letter_id, 1, 29.3))
    op.execute(insert_details.format(uuid.uuid4(), letter_id, 2, 32))
    op.execute(insert_details.format(uuid.uuid4(), letter_id, 3, 35))
