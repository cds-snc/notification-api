...
def downgrade():
    op.add_column(
        "email_branding",
        sa.Column("banner_colour", sa.VARCHAR(length=7), autoincrement=False, nullable=True),
    )
    op.add_column(
        "email_branding",
        sa.Column("single_id_colour", sa.VARCHAR(length=7), autoincrement=False, nullable=True),
    )
