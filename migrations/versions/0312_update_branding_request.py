...
def upgrade():
    ...
    template_subject = (
        "Branding change request for ((service_name)) | Demande de changement d''image de marque pour ((service_name))"
    )

    op.execute(
        "UPDATE templates SET content = '{}', subject = '{}' WHERE id = '{}'".format(
            template_content, template_subject, template_id
        )
    )
    op.execute(
        "UPDATE templates_history SET content = '{}', subject = '{}' WHERE id = '{}'".format(
            template_content, template_subject, template_id
        )
    )
