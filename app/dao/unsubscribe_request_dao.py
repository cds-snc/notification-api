...
def get_unsubscribe_requests_statistics_dao(service_id):
    """Returns count of actionable (unprocessed) unsubscribe requests for a service."""
    return (
        db.session.query(
...
def get_unsubscribe_request_reports_dao(service_id):
    """Returns batched reports for a service that have at least one request associated."""
    return (
...
def get_unsubscribe_requests_data_for_download_dao(service_id, batch_id):
    """Joins across both Notification tables to capture rows past the retention period."""
    results = []
...
def get_service_ids_with_unsubscribe_requests():
    return {row.service_id for row in UnsubscribeRequest.query.with_entities(UnsubscribeRequest.service_id).distinct()}


def dao_archive_batched_unsubscribe_requests(service_id):
    """Archives batched unsubscribe requests whose report was created > 7 days ago."""
...
def dao_archive_old_unsubscribe_requests(service_id):
    """Archives old unbatched unsubscribe requests older than 90 days."""
    return archive_unsubscribe_requests_from_query(
        UnsubscribeRequest.query.filter(
...
def archive_unsubscribe_requests_from_query(query):
    """Moves unsubscribe requests to history table (strips email_address for privacy)."""
