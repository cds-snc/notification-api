...
template_folder_blueprint = Blueprint("template_folder", __name__, url_prefix="/service/<uuid:service_id>/template-folder")
...
def move_to_template_folder(service_id, target_template_folder_id=None):
    ...
    for template_id in data["templates"]:
        try:
            ...
            msg = "Could not move to folder: No template found with id {} for service {}".format(template_id, service_id)
            raise InvalidRequest(msg, status_code=400)
