...
def create_template(service_id):
    ...
    if over_limit:
        ...
        if new_template.template_type == EMAIL_TYPE:
            current_app.logger.info({"error": "email_char_count_exceeded", "message": message, "service_id": service_id})
...
def update_template(service_id, template_id):
    ...
    if over_limit:
        ...
        if fetched_template.template_type == EMAIL_TYPE:
            current_app.logger.info({"error": "email_char_count_exceeded", "message": message, "template_id": template_id})
        raise InvalidRequest(errors, status_code=400)
