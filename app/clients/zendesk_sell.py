...
class ZenDeskSell(object):
    ...
    def _name_split(name: str) -> Tuple[str, str]:
        ...
        return (
            " ".join(name_tokenised[:-1]) if len(name_tokenised) > 1 else "",
            name_tokenised[-1],
        )

    @staticmethod
    def _generate_lead_data(
        ...
        return {
            "data": {
                ...
                "custom_fields": {
                    ...
                    if contact.intended_recipients in recipients
                    else "No value",
                },
            }
        }
    ...
    @staticmethod
    def _generate_note_data(resource_type: NoteResourceType, resource_id: str, content: str) -> Dict[str, Dict[str, str]]:
        return {
            "data": {
    ...
    def _send_request(
        self, method: str, relative_url: str, data: str = None
    ) -> Tuple[requests.models.Response, Optional[Exception]]:
    ...
    def search_lead_id(self, user: User) -> Optional[str]:
        resp, e = self._send_request(method="GET", relative_url=f"/v2/leads?email={user.email_address}")
        if e:
    ...
    def upsert_contact(self, user: User, contact_id: Optional[str]) -> Tuple[Optional[str], bool]:
        ...
        if contact_id:
            ...
            resp, e = self._send_request(method="PUT", relative_url=f"/v2/contacts/{contact_id}", data=data)
        else:
            resp, e = self._send_request(
                method="POST",
        ...
        try:
            ...
            return (
                resp_data["data"]["id"],
                resp_data["data"]["created_at"] == resp_data["data"]["updated_at"],
            )
    ...
    def upsert_deal(self, contact_id: str, service: Service, stage_id: int) -> Optional[str]:
        ...
        resp, e = self._send_request(
            method="POST",
            relative_url=f"/v2/deals/upsert?" f"custom_fields[notify_service_id]={str(service.id)}",
    ...
    def create_note(self, resource_type: NoteResourceType, resource_id: str, contact: ContactRequest) -> Optional[str]:
        ...
        resp, e = self._send_request(
            ...
            data=json.dumps(
                ZenDeskSell._generate_note_data(
                    resource_type,
                    resource_id,
                    ZenDeskSell._generate_note_content(contact),
                )
            ),
        )
        if e:
    ...
    def send_create_service(self, service: Service, user: User) -> Optional[str]:
        try:
            ...
            if contact_id:
                return self._common_create_or_go_live(
                    service,
                    user,
