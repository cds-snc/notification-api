...
def test_process_ses_results_in_complaint(sample_email_template):
    ...
    handle_complaint(json.loads(ses_complaint_callback()["Messages"])[0])
...
def test_handle_complaint_does_raise_exception_if_notification_not_found(notify_api):
    response = json.loads(ses_complaint_callback()["Messages"])[0]
...
class TestBounceRates:
    ...
    def test_bounce_types(self, notify_api, bounceType, bounceSubType, expected_bounce_classification):
        if bounceType == "Permanent":
            bounce_message = json.loads(ses_hard_bounce_callback(reference="ref", bounce_subtype=bounceSubType)["Messages"])[0]
        elif bounceType == "Transient" or bounceType == "Undetermined":
            bounce_message = json.loads(ses_soft_bounce_callback(reference="ref", bounce_subtype=bounceSubType)["Messages"])[0]
