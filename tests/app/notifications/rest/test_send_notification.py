...
def test_should_not_send_notification_if_restricted_and_not_a_service_user(
    notify_api, sample_template, sample_email_template, mocker, template_type, to, fake_uuid
):
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            mocked = mocker.patch('app.celery.provider_tasks.deliver_{}.apply_async'.format(template_type))
...
def test_should_send_notification_if_restricted_and_a_service_user(
    notify_api, sample_template, sample_email_template, template_type, mocker, sample_sms_sender
):
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            mocked = mocker.patch('app.celery.provider_tasks.deliver_{}.apply_async'.format(template_type))
...
def test_should_not_allow_template_from_another_service(
    notify_api, service_factory, sample_user, mocker, template_type, fake_uuid
):
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            mocked = mocker.patch('app.celery.provider_tasks.deliver_{}.apply_async'.format(template_type))
...
def test_should_block_api_call_if_over_day_limit_for_live_service(notify_db_session, notify_api, mocker):
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            mocker.patch(
...
@freeze_time('2016-01-01 12:00:00.061258')
def test_should_block_api_call_if_over_day_limit_for_restricted_service(notify_db_session, notify_api, mocker):
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
...
def test_should_allow_api_call_if_under_day_limit_regardless_of_type(
    notify_api, sample_user, mocker, restricted, sample_sms_sender
):
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            mocker.patch('app.celery.provider_tasks.deliver_sms.apply_async')
...
def test_should_not_send_sms_if_team_api_key_and_not_a_service_user(
    notify_api, sample_template, mocker, sample_sms_sender
):
    with notify_api.test_request_context(), notify_api.test_client() as client:
        mocker.patch('app.celery.provider_tasks.deliver_sms.apply_async')
...
def test_should_send_email_if_team_api_key_and_a_service_user(client, sample_email_template, fake_uuid, mocker):
    ...
    data = {'to': sample_email_template.service.created_by.email_address, 'template': sample_email_template.id}
    auth_header = create_authorization_header(service_id=sample_email_template.service_id, key_type=KEY_TYPE_TEAM)
...
def test_should_send_sms_if_team_api_key_and_a_service_user(
    client, sample_template, fake_uuid, mocker, sample_sms_sender
):
    mocked = mocker.patch('app.celery.provider_tasks.deliver_sms.apply_async')
    mocker.patch('app.notifications.process_notifications.uuid.uuid4', return_value=fake_uuid)
    ...
    api_key = ApiKey(
        service=sample_template.service, name='team_key', created_by=sample_template.created_by, key_type=KEY_TYPE_TEAM
    )
    save_model_api_key(api_key)
    auth_header = create_jwt_token(secret=api_key.secret, client_id=str(api_key.service_id))
...
def test_should_persist_notification(
    client, sample_template, sample_email_template, fake_uuid, mocker, template_type, queue_name, sample_sms_sender
):
    mocked = mocker.patch('app.celery.provider_tasks.deliver_{}.apply_async'.format(template_type))
    mocker.patch('app.notifications.process_notifications.uuid.uuid4', return_value=fake_uuid)
    ...
    api_key = ApiKey(service=template.service, name='team_key', created_by=template.created_by, key_type=KEY_TYPE_TEAM)
    save_model_api_key(api_key)
    auth_header = create_jwt_token(secret=api_key.secret, client_id=str(api_key.service_id))
...
def test_should_delete_notification_and_return_error_if_sqs_fails(
    client, sample_email_template, sample_template, fake_uuid, mocker, template_type, queue_name, sample_sms_sender
):
    mocked = mocker.patch(
...
def test_should_not_persist_notification_or_send_sms_if_simulated_number(
    client, to_sms, sample_template, mocker, sample_sms_sender
):
...
def test_should_not_send_notification_to_non_whitelist_recipient_in_trial_mode(
    client, sample_service_whitelist, notification_type, to, key_type, mocker, sample_sms_sender
):
...
def test_should_send_notification_to_whitelist_recipient(
    client, sample_service, notification_type, to, key_type, service_restricted, mocker, sample_sms_sender
):
...
@pytest.mark.parametrize(
    ...
    [(EMAIL_TYPE, SMS_TYPE, 'notify@digital.cabinet-office.gov.uk'), (SMS_TYPE, EMAIL_TYPE, '+16502532222')],
)
def test_should_error_if_notification_type_does_not_match_template_type(
    client, sample_service, template_type, notification_type, to, sample_sms_sender
):
...
def test_create_template_raises_invalid_request_when_content_too_large(sample_service, template_type, should_error):
...
def test_send_notification_uses_priority_queue_when_template_is_marked_as_priority(
    client, sample_service, mocker, notification_type, send_to, sample_sms_sender
):
...
def test_send_sms_returns_a_429_limit_exceeded_if_sms_sender_rate_limit_exceeded(client, sample_service, mocker):
    ...
    data = {'to': '6502532222', 'template': str(sample.id)}
...
def test_should_allow_international_number_on_sms_notification(
    client, sample_service_full_permissions, mocker, sample_sms_sender
):
    mocker.patch('app.celery.provider_tasks.deliver_sms.apply_async')
...
def test_should_not_allow_sms_notifications_if_service_permission_not_set(
    client, mocker, sample_template_without_sms_permission, sample_sms_sender
):
    mocked = mocker.patch('app.celery.provider_tasks.deliver_sms.apply_async')
...
def test_post_notification_should_set_reply_to_text(
    client, sample_service, mocker, notification_type, recipient, sample_sms_sender
):
