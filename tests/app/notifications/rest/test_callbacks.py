...
@pytest.mark.skip(reason='Endpoint disabled and slated for removal')
def test_firetext_callback_should_set_status_technical_failure_if_status_unknown(client, mocker, sample_notification):
...
@pytest.mark.skip(reason='Endpoint disabled and slated for removal')
def test_callback_should_return_200_if_cannot_find_notification_id(notify_db, notify_db_session, client, mocker):
    mocker.patch('app.statsd_client.incr')
    missing_notification_id = uuid.uuid4()
    data = 'mobile=441234123123&status=0&time=2016-03-10 14:17:00&reference={}'.format(missing_notification_id)
    response = firetext_post(client, data)
...
def test_firetext_callback_should_update_notification_status(client, mocker, sample_notification):
    ...
    data = 'mobile=441234123123&status=0&time=2016-03-10 14:17:00&reference={}'.format(sample_notification.id)
    response = firetext_post(client, data)

    json_resp = json.loads(response.get_data(as_text=True))
    assert response.status_code == 200
    assert json_resp['result'] == 'success'
    assert json_resp['message'] == 'Firetext callback succeeded. reference {} updated'.format(sample_notification.id)
...
@pytest.mark.skip(reason='Endpoint disabled and slated for removal')
def test_firetext_callback_should_update_notification_status_failed(client, mocker, sample_template):
    mocker.patch('app.statsd_client.incr')
    ...
    data = 'mobile=441234123123&status=1&time=2016-03-10 14:17:00&reference={}'.format(notification.id)
    response = firetext_post(client, data)

    json_resp = json.loads(response.get_data(as_text=True))
    assert response.status_code == 200
    assert json_resp['result'] == 'success'
    assert json_resp['message'] == 'Firetext callback succeeded. reference {} updated'.format(notification.id)
...
def test_firetext_callback_should_update_notification_status_pending(client, sample_template, mocker):
    ...
    original = get_notification_by_id(notification.id)
    assert original.status == 'sending'
    data = 'mobile=441234123123&status=2&time=2016-03-10 14:17:00&reference={}'.format(notification.id)
    response = firetext_post(client, data)
...
@pytest.mark.skip(reason='Endpoint disabled and slated for removal')
def test_process_mmg_response_returns_200_when_cid_is_valid_notification_id(sample_notification, client, mocker):
    mocker.patch('app.celery.service_callback_tasks.send_delivery_status_to_service.apply_async')
    sample_notification.status = 'sending'
    data = json.dumps(
        {
...
def test_process_mmg_response_status_5_updates_notification_with_permanently_failed(
    ...
    data = json.dumps(
        {'reference': 'mmg_reference', 'CID': str(sample_notification.id), 'MSISDN': '447777349060', 'status': 5}
    )
...
def test_process_mmg_response_status_2_updates_notification_with_permanently_failed(
    ...
    sample_notification.status = 'sending'
    data = json.dumps(
        {'reference': 'mmg_reference', 'CID': str(sample_notification.id), 'MSISDN': '447777349060', 'status': 2}
    )
...
@pytest.mark.skip(reason='Endpoint disabled and slated for removal')
def test_process_mmg_response_status_4_updates_notification_with_temporary_failed(sample_notification, client, mocker):
    mocker.patch('app.celery.service_callback_tasks.send_delivery_status_to_service.apply_async')
    sample_notification.status = 'sending'

    data = json.dumps(
        {'reference': 'mmg_reference', 'CID': str(sample_notification.id), 'MSISDN': '447777349060', 'status': 4}
    )
...
def test_process_mmg_response_unknown_status_updates_notification_with_technical_failure(
    ...
    sample_notification.status = 'sending'
    data = json.dumps(
        {'reference': 'mmg_reference', 'CID': str(sample_notification.id), 'MSISDN': '447777349060', 'status': 10}
    )
