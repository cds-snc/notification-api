...
def test_normal_api_key_returns_notifications_created_from_jobs_and_from_api(
    client, notify_db, notify_db_session, sample_api_key, sample_notification
):
    api_notification = create_sample_notification(notify_db, notify_db_session, api_key=sample_api_key)
...
def test_get_all_notifications_only_returns_notifications_of_matching_type(
    ...
    normal_notification = create_sample_notification(
        notify_db, notify_db_session, api_key=normal_api_key, key_type=KEY_TYPE_NORMAL
    )
    team_notification = create_sample_notification(notify_db, notify_db_session, api_key=team_api_key, key_type=KEY_TYPE_TEAM)
    test_notification = create_sample_notification(notify_db, notify_db_session, api_key=test_api_key, key_type=KEY_TYPE_TEST)
...
@pytest.mark.parametrize("key_type", [KEY_TYPE_NORMAL, KEY_TYPE_TEAM, KEY_TYPE_TEST])
def test_do_not_return_job_notifications_by_default(client, sample_template, sample_job, key_type):
...
def test_only_normal_api_keys_can_return_job_notifications(
    client, notify_db, notify_db_session, sample_service, sample_job, key_type
):
    team_api_key = ApiKey(
        service=sample_service,
        name="team_api_key",
    ...
    create_sample_notification(notify_db, notify_db_session, job=sample_job)
    normal_notification = create_sample_notification(
        notify_db, notify_db_session, api_key=normal_api_key, key_type=KEY_TYPE_NORMAL
    )
    team_notification = create_sample_notification(notify_db, notify_db_session, api_key=team_api_key, key_type=KEY_TYPE_TEAM)
