...
def test_dao_get_letter_contacts_by_service_id_does_not_return_archived_contacts(
    ...
    archived_contact = create_letter_contact(service=service, contact_block="London, E1 8QS", is_default=False, archived=True)
...
def test_add_letter_contact_with_no_default_is_fine(notify_db_session):
    service = create_service()
    letter_contact = add_letter_contact_for_service(service_id=service.id, contact_block="Swansea, SN1 3CC", is_default=False)
...
def test_dao_get_letter_contact_by_id_raises_sqlalchemy_error_when_letter_contact_does_not_exist(
    sample_service,
):
    with pytest.raises(SQLAlchemyError):
