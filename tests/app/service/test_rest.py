...
def test_update_service_with_valid_provider(
    ...
    resp = admin_request.post('service.update_service', service_id=sample_service.id, _data=data, _expected_status=200)
...
def test_should_not_update_service_with_inactive_provider(
    admin_request, notify_db, notification_type, sample_service, fake_uuid, mocker
):
...
def test_should_not_update_service_with_nonexistent_provider(
    admin_request, notify_db_session, notification_type, sample_service, fake_uuid
):
    data = {f'{notification_type}_provider_id': str(fake_uuid)}
...
def test_get_notification_for_service_without_uuid(client, notify_db, notify_db_session):
    service_1 = create_service(service_name='1', email_from='1')
    response = client.get(
        path='/service/{}/notifications/{}'.format(service_1.id, 'foo'), headers=[create_authorization_header()]
    )
    assert response.status_code == 404
...
def test_get_services_with_detailed_flag_excluding_from_test_key(notify_api, sample_template):
    ...
    with notify_api.test_request_context(), notify_api.test_client() as client:
        resp = client.get('/service?detailed=True&include_from_test_key=False', headers=[create_authorization_header()])
...
def test_get_services_with_detailed_flag_defaults_to_today(client, mocker):
    mock_get_detailed_services = mocker.patch('app.service.rest.get_detailed_services', return_value={})
    resp = client.get(url_for('service.get_services', detailed=True), headers=[create_authorization_header()])
...
def test_search_for_notification_by_to_field_filters_by_statuses(client, sample_template):
    ...
    notification2 = create_notification(
        sample_template, to_field='+16502532222', status='sending', normalised_to='+16502532222'
    )
...
def test_send_one_off_notification(sample_service, admin_request, mocker):
    ...
    response = admin_request.post(
        ...
        service_id=sample_service.id,
        _data={'template_id': str(template.id), 'to': '+16502532222', 'created_by': str(sample_service.created_by_id)},
...
def test_get_email_reply_to_addresses_when_there_are_no_reply_to_email_addresses(client, sample_service):
    response = client.get(
        '/service/{}/email-reply-to'.format(sample_service.id), headers=[create_authorization_header()]
    )
...
def test_get_email_reply_to_addresses_with_one_email_address(client, notify_db, notify_db_session):
    ...
    response = client.get('/service/{}/email-reply-to'.format(service.id), headers=[create_authorization_header()])
...
def test_create_smtp_relay_for_service_if_it_already_has_one(client, notify_db, notify_db_session):
    ...
    resp = client.post('/service/{}/smtp'.format(service.id), headers=[create_authorization_header()])
...
def test_get_smtp_relay_for_service(mocker, client, notify_db, notify_db_session):
    ...
    resp = client.get('/service/{}/smtp'.format(service.id), headers=[create_authorization_header()])
...
def test_delete_smtp_relay_for_service_returns_500_if_none(mocker, client, notify_db, notify_db_session):
    ...
    resp = client.delete('/service/{}/smtp'.format(service.id), headers=[create_authorization_header()])
