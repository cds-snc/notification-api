...
def test_delete_dvla_response_files_older_than_seven_days_removes_old_files(notify_api, mocker):
    ...
    mocker.patch(
        "app.celery.nightly_tasks.s3.get_s3_bucket_objects",
        return_value=single_page_s3_objects[0]["Contents"],
    )
    ...
    remove_s3_mock.assert_has_calls(
        [
            call(
                ...
                single_page_s3_objects[0]["Contents"][0]["Key"],
            ),
            call(
                current_app.config["DVLA_RESPONSE_BUCKET_NAME"],
                single_page_s3_objects[0]["Contents"][1]["Key"],
            ),
        ]
    )
