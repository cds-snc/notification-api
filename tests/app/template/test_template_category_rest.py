...
def test_should_create_new_template_category(client, notify_db, sample_user, notify_db_session):
    data = {
        "name_en": "new english",
        "name_fr": "new french",
    ...
    assert response.json["template_category"]["created_by_id"] == str(sample_user.id)


@pytest.mark.parametrize("key, updated_value", [("name_en", "updated english"), ("name_fr", "updated_french")])
def test_post_create_template_categories_returns_400_if_name_is_duplicate(
    key, updated_value, admin_request, sample_user, notify_db, notify_db_session
):
    data = {
        "name_en": "new english",
        "name_fr": "new french",
...
def test_post_update_template_category_returns_400_if_name_is_duplicate(
    admin_request, key, sample_user, sample_template_category
):
    data = {
        "name_en": "new english",
        "name_fr": "new french",
...
def test_get_template_category_by_id(client, sample_template_category):
    ...
    assert response.json["template_category"]["created_by_id"] == str(sample_template_category.created_by_id)
