...
def _post_notification(client, template, url, to, sms_sender_id=None):
    ...
    return client.post(path=url, data=json.dumps(data), headers=[('Content-Type', 'application/json'), auth_header])
...
def test_post_sms_contract(client, mocker, sample_template, sample_sms_sender):
    ...
    response_json = return_json_from_response(
        _post_notification(
            client, sample_template, url='/notifications/sms', to='6502532222', sms_sender_id=str(sample_sms_sender.id)
        )
    )
