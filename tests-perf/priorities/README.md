...
If you use the suffix "-bulk" for your reference / name then you can get statistics using the following Hasura query:
...
bulk_initial_data as (
    select 
        to_timestamp(split_part(j.original_file_name, ' ', 1), 'YYYY-MM-DD"T"HH24:MI:SS.US') as posted_at,
...
bulk_stats as ( 
    select
        '/bulk' endpoint,
        notification_type, status, priority, count(*),
